...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .timeline-item.changed { border-left-color: var(--blue); }
    @media (max-width: 820px) {
      h1 { font-size: 22px; }
      main { padding-left: 12px; padding-right: 12px; }
      .grid, .two, .toolbar, .live-strip, .behavior-head, .validation-strip, .timeline { grid-template-columns: 1fr; }
      .card { padding: 12px; }
      canvas { height: 180px; }
    }
  </style>
  ...
  <script>
    ...
    function renderBehaviorArchive() {
      ...
      document.getElementById('behaviorArchive').innerHTML = `
        ...
        <div class="timeline">${evolutionCards(payload.evolution || {})}</div>
    ...
    function driverList(drivers) {
      if (!drivers.length) return '<div class="subtitle">暂无证据权重排序。</div>';
      return `<div class="driver-list">
        ${drivers.slice(0, 5).map(driver => `<div class="driver-row">
          <span class="muted">${escapeHtml(driver.stars || '')}</span>
          <span><span class="driver-name">${escapeHtml(driver.name || '-')}</span><br><span class="muted">${escapeHtml(driver.reason || '')}</span></span>
          <span class="pill">${escapeHtml(driver.direction || '中性')}</span>
        </div>`).join('')}
      </div>`;
    }
    ...
    function evolutionCards(evolution) {
      const html = ASSETS.map(asset => {
        const points = evolution[asset] || [];
        if (!points.length) {
          return `<div class="behavior-panel"><div class="label">${asset} 行为演化</div><div class="muted">暂无历史轨迹</div></div>`;
        }
        return `<div class="behavior-panel">
          <div class="label">${asset} 行为演化 · 最近${points.length}条</div>
          <div class="timeline-list">${points.map(point => `<div class="timeline-item ${point.changed ? 'changed' : ''}">
            <div><strong>${escapeHtml(shortDate(point.date))}</strong> · ${escapeHtml(point.phase || '-')} · 综合 ${plain(point.composite, '-')}</div>
            <div class="subtitle">主导因素：${escapeHtml(point.driver || '-')} ${escapeHtml(point.driver_stars || '')}</div>
            <div class="tags">${(point.new_tags || []).map(tag => `<span class="tag">${escapeHtml(tag)}</span>`).join('')}</div>
          </div>`).join('')}</div>
        </div>`;
      }).join('');
      return html || '<div class="behavior-panel"><div class="label">行为演化</div><div class="muted">暂无历史轨迹</div></div>';
    }
    ...
    function buildContext(latestByAsset, useLive) {
      ...
      const behaviorAssets = ASSETS.map(asset => {
        ...
        const confidence = item.confidence || {};
        const drivers = (item.drivers || []).slice(0, 5).map(driver => `${driver.stars || ''} ${driver.name || '-'}（${driver.direction || '中性'}）：${driver.reason || '-'}`).join('\\n');
...
关键证据: ${flattenEvidence(item.evidence).slice(0, 5).join('；')}`;
      ...
      const evolutionText = ASSETS.map(asset => {
        const points = behavior.evolution?.[asset] || [];
        const lines = points.map(point => `${point.date}: ${point.phase} -> ${point.driver}${point.new_tags?.length ? ' / 新标签 ' + point.new_tags.join('、') : ''}`);
        return `${asset}\\n${lines.join('\\n') || '-'}`;
      }).join('\\n\\n---\\n\\n');
...
请基于以上上下文分析 BTC/ETH/WLD 的资金行为、异常事件、相对强弱、风险和下一步待验证假设。不要给自动交易指令。`;
    ...
    function shortDate(value) { return value ? String(value).slice(5, 10) : '-'; }
