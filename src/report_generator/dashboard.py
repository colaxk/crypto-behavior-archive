  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 820px) {
      ...
      .grid, .two, .toolbar { grid-template-columns: 1fr; }
      .card { padding: 12px; }
      canvas { height: 180px; }
    }
  </style>
...
<body>
  ...
  <main>
    ...
    <section class="card single">
      ...
      <div class="subtitle">蓝色 BTC，紫色 ETH，绿色 WLD。数据来自 docs/data/prices_*.json。</div>
    </section>
  ...
  <footer>GitHub Pages 静态页面。历史数据由 GitHub Actions commit 到仓库中的 CSV / JSON / Markdown 文件。</footer>

  <script>
    const ASSETS = ['BTC', 'ETH', 'WLD'];
    const COLORS = { BTC: '#2458d3', ETH: '#7c3aed', WLD: '#12805c' };
    const SYMBOLS = { BTC: 'BTCUSDT', ETH: 'ETHUSDT', WLD: 'WLDUSDT' };
    const state = { snapshots: [], prices: {}, events: [], reports: [], hypotheses: {}, selectedDate: null };
    ...
    function render() {
      const latestByAsset = Object.fromEntries(ASSETS.map(asset => [asset, snapshotFor(asset, state.selectedDate)]));
      document.getElementById('subtitle').innerText = `历史数据日期 ${state.selectedDate} · 最新仓库快照 ${state.hypotheses.latest_snapshot_time || '-'}`;
      document.getElementById('assetCards').innerHTML = ASSETS.map(asset => assetCard(asset, latestByAsset[asset])).join('');
      renderEvents();
      renderReports();
      drawChart();
      document.getElementById('gptContext').innerText = buildContext(latestByAsset);
    }
    ...
    function assetCard(asset, row) {
      if (!row) return `<div class="card"><h2>${asset}</h2><div class="muted">暂无快照</div></div>`;
      return `<div class="card">
        <div class="asset-head"><h2>${asset}</h2><span class="pill">${shortTime(row.timestamp)}</span></div>
        <div class="metric-grid">
          ...
          ${metric('爆仓总额', compact(row.liquidation_total, '未返回'))}
          ${metric('多单爆仓', compact(row.long_liquidation, '未返回'))}
          ${metric('空单爆仓', compact(row.short_liquidation, '未返回'))}
          ${metric('多空比', plain(row.long_short_ratio))}
          ${metric('CVD', compact(row.cvd, '未返回'))}
          ${metric('Heatmap', plain(row.heatmap, '未接入'))}
          ${metric('现货成交量', compact(row.spot_volume))}
          ${metric('合约成交量', compact(row.futures_volume))}
        </div>
        <div class="subtitle">${escapeHtml(row.note || '')}</div>
      </div>`;
    }
    ...
    function buildContext(latestByAsset) {
      const blocks = ASSETS.map(asset => {
        const row = latestByAsset[asset] || {};
        return `${asset}

价格: ${money(row.price)} / 24h ${pct(row.change_24h)} / 日期 ${day(row.timestamp) || '-'}
OI: ${compact(row.oi)}
Funding: ${pct(row.funding_rate, '未返回')}
Liquidation: ${compact(row.liquidation_total, '未返回')}
CVD: ${compact(row.cvd, '未返回')}
Heatmap: ${plain(row.heatmap, '未接入')}
状态: ${row.note || '待判断'}`;
      }).join('\\n\\n---\\n\\n');
...
请基于以上上下文分析 BTC/ETH/WLD 的资金行为、异常事件、相对强弱、风险和下一步待验证假设。不要给自动交易指令。`;
    ...
    function pct(value, missing = '-') {
      ...
      return Number.isFinite(number) ? number.toFixed(2) + '%' : missing;
    }
    function money(value) {
      if (value === null || value === undefined || value === '') return '-';
      const number = Number(value);
      if (!Number.isFinite(number)) return '-';
      return number >= 100 ? '$' + number.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '$' + number.toFixed(4);
    }
    function compact(value, missing = '-') {
      ...
      for (const [suffix, divisor] of units) {
        if (number >= divisor) return sign + (number / divisor).toFixed(2) + suffix;
      }
      return sign + number.toPrecision(4);
    }
